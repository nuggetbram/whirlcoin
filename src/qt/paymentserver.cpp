...
const QString BITCOIN_IPC_PREFIX("whirlcoin:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click whirlcoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start whirlcoin: click-to-pay handler");
    else
...
{
    // clicking on whirlcoin: URLs creates FileOpen events on the Mac:
