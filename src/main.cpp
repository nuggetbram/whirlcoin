...
// Copyright (c) 2013 WhirlCoin developers
...
const string strMessageMagic = "WhirlCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    double block = nHeight * 1.0 + 1.0 - 1.0;

    nSubsidy = -sqrt(block/347.000) + 100 * COIN;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 6 * 60; // 6 minutes
static const int64 nTargetSpacing = 45; // 0.75 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 8 blocks

static const int64 nAveragingInterval = nInterval * 2; // 16 blocks
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WhirlCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WhirlCoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running WhirlCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            }
    ...
    {
        printf("WhirlCoinMiner terminated\n");
        throw;
    }
}
